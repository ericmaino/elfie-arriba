    ...
    public class DefaultCrawler : IServiceIdentity
    {
        ...
        public async Task Crawl(IItemProvider provider, IItemConsumer consumer)
        {
            ...
            finally
            {
                ...
                // Old tracing logic
                Log.WriteLine(string.Format("Crawler Done. At {1:u}, {2:n0} items, {3} read, {4} write, {5} save for '{0}'.", ConfigurationName, DateTime.Now, itemCount, readWatch.Elapsed.ToFriendlyString(), writeWatch.Elapsed.ToFriendlyString(), saveWatch.Elapsed.ToFriendlyString()));
            }
        }
